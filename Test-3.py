...
def convert():
    ...
    if is_binary_input:
        ...
        try:
            ...
            # Label to display the number of bits
            convert.bits_label = ttk.Label(result_text, text=f"Number of Bits: {len(binary_value)}", style="TLabel")
            result_text.window_create(tk.END, window=convert.bits_label)
...
result_text.grid(column=0, row=4, pady=5, sticky=tk.W)
